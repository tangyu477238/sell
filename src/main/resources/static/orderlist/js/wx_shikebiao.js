...
(function($) {
    ...
    appcan.ready(function() {
        ...
        var datestr ='<div id ="date1" class="ubb-01 ulev-app2 t-gra-63 swidth3" onclick="setHoliday(1)">订单信息</div>'
        +'<div id ="date0" class="ubb-01 ulev-app2 t-gra-63 swidth3" onclick="setHoliday(0)">预约记录</div>';
        $("#date").html(datestr);
...
function getBanci() {
    ...
    var url = mesPath+'/verification/shikebiao?route='+route+'&holiday='+holiday;
    uexWindow.toast('1','5',"数据加载中...",'');
    reqAJAX(url,function(res){   
        uexWindow.closeToast();
        var tmp=getType(res)=='string'&&isDefine(res)?JSON.parse(res):res; 
