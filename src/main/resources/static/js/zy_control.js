...
function zy_conF(id,url,x,y)
{
	var s=window.getComputedStyle($$(id),null);
	uexWindow.openPopover(id,"0",url,"",int(x),int(y),int(s.width),int(s.height),int(s.fontSize),"4");
}
function zy_conr(id,url,x,y,disBottom)
{
	var s=window.getComputedStyle($$(id),null);
	uexWindow.openPopover(id,"0",url,"",int(x),int(y),int(s.width),int(s.height),int(s.fontSize),"0",disBottom);
}
...
function isios(){
	if (uexWidgetOne.platformName=="iOS"){
		var ver = uexWidgetOne.platformVersion;
		if(ver==""){return;}
		var arr=ver.split(".");
		if (arr[0] > 6) {
			$$("iostop").style.display = '';
			uexWindow.setStatusBarTitleColor('0');
		}
	}
	else{
		$$("iostop").style.display='none';
	}
}
